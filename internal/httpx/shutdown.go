...
//
// It creates a child context with the given deadline from the caller's context,
// calls srv.Shutdown, and returns any shutdown error. If the caller's context is
// already cancelled or deadlined, that cancellation propagates immediately.
// The srv.Close is called as a fallback only when the deadline context expires —
// http.Server.Shutdown handles this internally.
//
